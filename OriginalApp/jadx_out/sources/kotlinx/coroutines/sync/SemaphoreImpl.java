...
public final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cancellableContinuation) {
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (nextOrClosed == ConcurrentLinkedListKt.CLOSED) {
                        m1525constructorimpl = SegmentOrClosed.m1525constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    m1525constructorimpl = SegmentOrClosed.m1525constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (SegmentOrClosed.m1530isClosedimpl(m1525constructorimpl)) {
                break;
            }
            Segment m1528getSegmentimpl = SegmentOrClosed.m1528getSegmentimpl(m1525constructorimpl);
            while (true) {
                Segment segment = (Segment) this.tail;
                if (segment.getId() >= m1528getSegmentimpl.getId()) {
                    break;
                } else if (!m1528getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                    z = false;
                    continue;
                    break;
                } else if (SafePublicationLazyImpl$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment, m1528getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (m1528getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                    m1528getSegmentimpl.remove();
                }
            }
            z = true;
            continue;
        } while (!z);
        SemaphoreSegment semaphoreSegment4 = (SemaphoreSegment) SegmentOrClosed.m1528getSegmentimpl(m1525constructorimpl);
        ...
        if (!semaphoreSegment4.acquirers.compareAndSet(i3, null, cancellableContinuation)) {
            ...
            if (semaphoreSegment4.acquirers.compareAndSet(i3, symbol, symbol2)) {
